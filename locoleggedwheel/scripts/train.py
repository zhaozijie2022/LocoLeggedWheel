...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlOnPolicyRunnerCfg):
    ...
    if agent_cfg.resume:
        ...
        if agent_cfg.pretrained:
            resume_log_root_path = os.path.join("logs", "rsl_rl", agent_cfg.resume_experiment)
            resume_log_root_path = os.path.abspath(resume_log_root_path)
        # get path to previous checkpoint
        resume_path = get_checkpoint_path(resume_log_root_path, agent_cfg.load_run, agent_cfg.load_checkpoint)
        print(f"[INFO]: Loading model checkpoint from: {resume_path}")
        # load previously trained model
        runner.load(resume_path, pretrained=agent_cfg.pretrained)
